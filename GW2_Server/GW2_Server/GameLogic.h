...
public:
	...
	Vector3 operator-(const Vector3& other) const {	return Vector3(_x - other._x, _y - other._y, _z - other._z); }
	Vector3 operator*(const Vector3& other) const { return Vector3(_x * other._x, _y * other._y, _z * other._z); }
	Vector3 operator*(const float& other) const { return Vector3(_x * other, _y * other, _z * other); }
	Vector3 operator/(const Vector3& other) const {	return Vector3(_x / other._x, _y / other._y, _z / other._z); }
	Vector3 operator/(const float& other) const {	return Vector3(_x / other, _y / other, _z / other); }
	bool operator==(const Vector3& other) const { return _x == other._x && _z == other._z; }

public:
	static float GetMagnitude(Vector3 v);
	static Vector3 GetNormalVector(Vector3 v);
	float GetDistance(const Vector3& other) const { return (float)std::sqrt((_x - other._x) * (_x - other._x) + (_z - other._z) * (_z - other._z)); }
	...
	float Dot(const Vector3& o) const { return _x * o._x + _y * o._y + _z * o._z; }
	float Length() const;
...
struct HitResult
{
	float t;			// 레이 파라미터
	float u, v;			// 바리센트릭 좌표
	Vector3 position;	// 교차 지점
	Vector3 normal;		// 삼각형 법선
};
